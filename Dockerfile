# Docker file for dcm_tagExtract ChRIS plugin app
#
# Build with
#
#   docker build -t <name> .
...
#
#   docker run -ti -e HOST_IP=$(ip route | grep -v docker | awk '{if(NF==11) print $9}') --entrypoint /bin/bash local/pl-pfdicom_tagExtract
#
